...
/**
 * Class used for sending different emails for each purpose needed.
 */
@Service
@AllArgsConstructor
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    @Async
    public void sendEmailIfSubscriptionExpired(String to, String body) {
        try {
            MimeMessage mimeMessage = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, "utf-8");
            helper.setText(body, true);
    ...
    @Override
    @Async
    public void sendTraining(String to, String body) {
        try {
            MimeMessage mimeMessage = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, "utf-8");
            helper.setText(body, true);
            helper.setTo(to);
            helper.setSubject("Requested training plan");
            helper.setFrom("primefittool@gmail.com");
            mailSender.send(mimeMessage);
        } catch (MessagingException e) {
            LOGGER.error("Failed to send email", e);
